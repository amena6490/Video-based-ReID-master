...
def get_full_clips_re_id(data_dir, num_clips,ep_dirs, num_rec_out=1):  
    """
    Loads a batch of random clips from the unprocessed train or test data.
    ...
    for clip_num,ep_dir in enumerate(ep_dirs):
    		ep_frame_paths=sorted(glob(os.path.join(ep_dir,'*')))
    		start_index=(len(ep_frame_paths)-(c.HIST_LEN + 1 - 1))  #num_rec_out
...
def get_test_batch_re_id(test_batch_size,ep_dirs, num_rec_out=1):
    """
    Gets a clip from the test dataset.
    ...
    @return: An array of shape:
             ...
             A batch of frame sequences with values normalized in range [-1, 1].
    """
    return get_full_clips_re_id(c.TEST_DIR,test_batch_size,ep_dirs,num_rec_out=num_rec_out)	  
