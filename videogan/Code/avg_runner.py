...
from utils import get_train_batch, get_test_batch,get_test_batch_re_id
...
class AVGRunner:
	...
	#step = 0
		...
		batch=get_test_batch_re_id(c.BATCH_SIZE,ep_dirs,num_rec_out=self.num_test_rec)
            	#print(batch)
		self.g_model.test_batch(batch,ep_dirs,
		self.global_step,num,num_rec_out=self.num_test_rec)
...
def main():
    ...
    load_path = None
    test_only = False
    num_test_rec = 20  # number of recursive predictions to make on test
    num_steps = 1000001
    try:
