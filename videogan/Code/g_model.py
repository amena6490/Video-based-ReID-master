...
class GeneratorModel:
    ...
    def define_graph(self):
		...
		loss_summary = tf.summary.scalar('train_loss_G', self.global_loss)
            ...
            self.summaries_train = tf.summary.merge(self.summaries_train)  #merge_summary
        ...
        if global_step % c.IMG_SAVE_FREQ == 0:
            ...
            for scale_num in xrange(self.num_scale_nets):
                ...
                for i, img in enumerate(gt_frames):
                    ...
                    sknorm_img = (img / 4) + 0.5
                    resized_frame = resize(sknorm_img, [scale_height, scale_width, 3])
                    scaled_gt_frames[i] = (resized_frame - 0.5) * 4
    ...
    def test_batch(self, batch, ep_dirs,global_step,step,num_rec_out=1, save_imgs=True):
        """
        Runs a training step using the global loss on each of the scale networks.
	...
	if dataset == 1:   #choice dataset 1:is ILIDS  2: is PRID2011

        	IMAGE_DIR = './i-LIDS-VID/cam2/'
		...
		if save_imgs:
			...
			for clip_num,ep_dir in enumerate(ep_dirs):
				#for pred_num in xrange(len(input_frames)):
				ep_dirnum=ep_dir[-9:]
				pred_dir=c.get_dir(os.path.join(IMAGE_DIR,str(ep_dirnum)))
				ep_frame_path=sorted(glob(os.path.join(ep_dir,'*')))
				last_frame=ep_frame_path[len(ep_frame_path)-1]
				last_frame=int(last_frame[-9:-4])
				for rec_num in xrange(num_rec_out):
					rec_num=int(rec_num)
					s=str(last_frame+rec_num+1).zfill(5)
					gen_img=rec_preds[rec_num][clip_num]
					imsave(os.path.join(pred_dir,'cam2_'+str(ep_dirnum)+'_'+s+'.png'),gen_img)
			print '_' * 30
	else:
		IMAGE_DIR = './PRID2011/cam_b/'
		if save_imgs:
			...
			#for pred_num in xrange(len(input_frames)):
				...
				ep_frame_path=sorted(glob(os.path.join(ep_dir,'*')))
				last_frame=ep_frame_path[len(ep_frame_path)-1]
				last_frame=int(last_frame[-8:-4])
				for rec_num in xrange(num_rec_out):
					rec_num=int(rec_num)
					s=str(last_frame+rec_num+1).zfill(4)
					gen_img=rec_preds[rec_num][clip_num]
					imsave(os.path.join(pred_dir,s+'.png'),gen_img)
			print '_' * 30
